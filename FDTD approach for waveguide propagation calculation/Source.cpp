...
void main()
{
	...
	int i, j;
	int step, Timestop;
	int Alter;                                       //在角点处理中用来选择变量
	int Jsource, Ismin, Ismax;
	double n0;
	double m[2] = { 1.0,3.0 };
	double Eps;
	double EAC[8][2][2];                             //用来存储角点
	double Ei;                                        //定义激励源
	double T1, T2;
	double ia, jb;
	double wavelength;    //定义波长
	double t;
	double r;                                              //弯曲波导半径  
	double d;                                              //波导宽度
	double temp;
	double PI = 3.1415926535895;
	double u = PI * 0.0000004;                                 //介质介电系数 
	double Eps0 = 8.85e-12;                            //磁导系数
	double c = 299792457.4;                               //光速
	double coeff_E;                          //电场计算的系数
	double coeff_H = 1 / c / u / 2;                           //磁场计算的系数
	...
	//if ((fp = fopen("trip1000.txt", "w")) == NULL)
